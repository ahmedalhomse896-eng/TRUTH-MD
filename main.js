...
async function handleOfflineCommand(sock, chatId, message, commandName) {
    const offlineMsg = `❌ *OFFLINE MODE*\n\nThe bot is currently offline due to network connectivity issues.\n\n🔄 *Command:* \`${commandName}\`\n⏰ *Status:* Network unavailable\n\nThe bot will automatically resume normal operation when internet connection is restored.`;
...
// Wrapper for API-dependent commands
async function withNetworkCheck(commandFunc, sock, chatId, message, ...args) {
    if (!(await checkNetworkConnectivity())) {
        const commandName = args[0] || 'unknown command';
        await handleOfflineCommand(sock, chatId, message, commandName);
        return false;
    }
    return true;
}
...
const { antibugCommand, isAntibugEnabled, handleAntibug } = require('./commands/antibug');
const { fontstyleCommand, isFontStyleEnabled, applyFontStyle, getCurrentFont } = require('./commands/fontstyle');
const { autoblockCommand, isAutoblockEnabled, handleAutoblock } = require('./commands/autoblock');
const { statusAntideleteCommand, isStatusAntideleteEnabled, storeStatus, handleStatusRevocation } = require('./commands/statusantidelete');
const { autoBioCommand, startAutoBio, stopAutoBio } = require('./commands/autobio');
const { autolikeCommand } = require('./commands/autolike');
const { autoviewCommand } = require('./commands/autoview');
...
const addApiCommand = require('./commands/addapi');
const listApisCommand = require('./commands/listapis');
...
const forkCommand = require('./commands/fork');
...
const { vvReplyCommand, vvDmCommand } = require('./commands/vvreply');
...
async function handleMessages(sock, messageUpdate, printLog ) {
    try {
        ...
        if (await handleAntibug(sock, message)) return;
        if (await handleAutoblock(sock, message)) return;
        ...
        if (message.message?.protocolMessage?.key && (protoType === 0 || protoType === undefined || protoType === null)) {
            ...
            await handleStatusRevocation(sock, message);
            return;
        }
...
const rawMessage = (
    ...
    ''
        ...
        if (userMessage && userMessage.startsWith(prefix)) {
            ...
            const msgType = Object.keys(message.message || {}).find(k => k !== 'messageContextInfo') || 'unknown';
            const senderNum = (message.key.participant || message.key.remoteJid || '').split('@')[0];
            const tz = 'Africa/Nairobi';
            const msgTime = new Date().toLocaleString('en-US', { weekday: 'long', hour: '2-digit', minute: '2-digit', second: '2-digit', timeZone: tz, hour12: true }) + ' EAT';
            const topBar = chalk.yellow('━━━━━━━━━━') + chalk.red('━━') + chalk.yellow(' 『 ') + chalk.green(' TRUTH-MD ') + chalk.yellow('』 ') + chalk.blue('━━') + chalk.yellow('━━━━━━━━━━');
            const bottomBar = chalk.green('━━━━━') + chalk.yellow('━━━━━━━') + chalk.red('━━━━━━━━━━━━━') + chalk.blue('━━━━━━━━') + chalk.cyan(' ~~');

            const originalSendMessage = sock.sendMessage;
            sock.sendMessage = async function(...args) {
                const responseTime = Date.now() - commandStartTime;
                const speedRating = responseTime < 500 ? 'FAST' : responseTime < 2000 ? 'NORMAL' : 'SLOW';
                console.log(topBar);
                console.log(chalk.yellow('»') + chalk.magenta(` Message Type: ${msgType}`));
                console.log(chalk.yellow('»') + chalk.yellow(` Message Time: ${msgTime}`));
                console.log(chalk.yellow('»') + chalk.cyan(` Delay: ${(responseTime / 1000).toFixed(2)}s [ ${speedRating} ]`));
                console.log(chalk.yellow('»') + chalk.cyan(` Sender: ${senderNum}`));
                console.log(chalk.yellow('»') + chalk.green(` Name: ${pushname}`));
                console.log(chalk.yellow('»') + chalk.blue(` Chat ID: ${chatId}`));
                console.log(chalk.yellow('»') + chalk.white(` Message: ${body.substring(0, 50)}${body.length > 50 ? '...' : ''}`));
        ...
        /*━━━━━━━━━━━━━━━━━━━━*/
        // Auto-send view-once to DM when replying with sticker (owner only)
        /*━━━━━━━━━━━━━━━━━━━━*/
        if (message.key.fromMe && message.message?.stickerMessage) {
            ...
            if (quoted) {
                ...
                if (viewOnceMsg) {
                    ...
                    if (mediaType && mediaMessage) {
                        try {
                            console.log(`[STICKER-VV-DM] Processing ${mediaType} from sticker reply`);
                            ...
                            const ownerJid = message.key.remoteJid; // Owner's DM

                            if (mediaType === 'image') {
                                await sock.sendMessage(ownerJid, {
                                    image: buffer,
                                    caption: appendWatermark(mediaMessage.caption || 'View-once media from sticker reply')
                                });
                                console.log('[STICKER-VV-DM] Image sent to owner DM');
                            } else if (mediaType === 'video') {
                                const tmpPath = path.join(TEMP_DIR, `sticker_vv_dm_${Date.now()}.mp4`);
                                ...
                                console.log('[STICKER-VV-DM] Video sent to owner DM');
                            }

                            await sock.sendMessage(chatId, { text: '✅ View-once media sent to your DM!' });
                            return;

                        } catch (error) {
                            console.error('Error in sticker view-once DM:', error);
                            await sock.sendMessage(chatId, {
...
case userMessage.startsWith(`${prefix}pair`):
    ...
    break;
            ...
            case userMessage.startsWith(`${prefix}antibug`):
                ...
                break;
            case userMessage.startsWith(`${prefix}fontstyle`):
                await fontstyleCommand(sock, chatId, senderId, message, userMessage, prefix);
                break;
            ...
            case userMessage.startsWith(`${prefix}autolike`):
                ...
                commandExecuted = true;
                break;
            case userMessage.startsWith(`${prefix}autoview`):

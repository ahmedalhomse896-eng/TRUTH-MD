...
async function imagineCommand(sock, chatId, message) {
    try {
        ...
        console.log(`🔄 Primary image API failed, trying fallbacks for prompt: "${imagePrompt.substring(0, 50)}..."`);
        const fallbackResult = await fallbackManager.tryFallbacks('image_generation', enhancedPrompt);

        if (fallbackResult.success) {
            await sock.sendMessage(chatId, {
                image: fallbackResult.data,
                caption: `🎨 Generated image (Fallback: ${fallbackResult.api}):\n"${imagePrompt}"`
            }, {
                quoted: message
            });
            return;
        ...
        // If all APIs fail, send error message
        await sock.sendMessage(chatId, {
            text: '❌ Failed to generate image with primary API.\n\n🔄 All fallback APIs also failed. Please try again later.'
        }, {
            quoted: message
        });
    }
